...
<body>
	<div class="container">
		...
		<c:choose>
			...
			<!-- 계산서 형식의 표 --><div class="col-sm-4">
			
		<form action="/order/basketorder" method="get">
				...
				<table class="table fixed-right">
					...
					<c:set var="total" value="0"/>
						<c:forEach items="${basket_list}" var="BasketVO">
							<tr>
								<td id="bno" data-bno="${BasketVO.basket.bno}">${BasketVO.products.product_name}</td>
								<td class="text-right">${BasketVO.basket.basket_price}원</td>						
							</tr>
							<input type="hidden" value="${BasketVO.basket.bno}" name="basketListbno[]"/>
					...
					<tfoot>
						...
						<tr>
							<td colspan="2" class="text-right"><button type="submit" id="btn_pay" class="btn btn-warning" style="width: 100%">결제하러가기</button></td>
						</tr>
					</tfoot>
				</table>
			</form>	
